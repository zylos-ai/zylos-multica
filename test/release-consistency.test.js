...
// Release rule (AGENTS.md §7): a release bumps four files in the same commit.
// These parsers + checker are pure so the suite can also run permanent
// negative controls against known-bad fixtures — the gate itself is tested.
...
function realFiles() {
  return {
    ...
    lockJson: fs.readFileSync(path.join(root, 'package-lock.json'), 'utf8'),
...
test('negative control: the v0.3.0 incident shape (stale SKILL.md frontmatter) is caught', () => {
  ...
  assert.match(mismatches[0], /SKILL\.md frontmatter version = 0\.2\.21/);
});
