...
3. **不自建重试**：投递失败不 start，留给服务端 recovery 窗口重派。**due-date 例外的闭环**（review P2 修订）：scheduler add 成功定义为 durable handoff（此时即 start），但 handoff 不等于送达——组件主循环须对自己登记的一次性 scheduler 任务做对账：发现终态 `failed`（错过 miss window 等）即调 Multica fail API（error 注明 scheduler 失败原因），把任务交还服务端重派，杜绝"scheduler 已死、Multica 永远 running"的跨账本悬挂。
   **对账接口落点**（review 追问收敛）：现有 scheduler CLI 无结构化 get/list（--json 不存在），禁止解析人类可读表格输出；也不引入自己的 sqlite 连接/schema 拷贝。采用：(a) 注册时 `--name` 用含完整 Multica task id 的确定性名称（`multica-task-<task_id>`），身份不靠解析 stdout；(b) 状态读取 import scheduler 自带模块 `~/zylos/.claude/skills/scheduler/scripts/database.js` 的 `getDb()`，只读 `SELECT id,status,last_error FROM tasks WHERE name=?`（同名多行取最新）。这是 API 缺字段场景下经组件自有模块的只读旁路（符合 API-first 例外条款），验收含一条对账查询真实命中的用例，schema 漂移会在验收炸响而非静默。upstream 结构化输出（`cli.js get/list --json`）已提 zylos-core issue，落地后迁移。SKILL.md `dependencies` 显式声明 `scheduler` + `comm-bridge`。
